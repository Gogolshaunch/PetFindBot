...
def new_human(person, chat_id):     
    if person.status == 1: 
        with Session(bind=engine2) as session:              
            human = Finder(chat_id, person.name, person.phonenumber, person.tg_user, person.city, person.type_pet, person.name_pet, person.sex_pet, person.breed_pet, person.describe, person.photo)  
    else:         
        with Session(bind=engine) as session:  
            human = Owner(chat_id, person.name, person.phonenumber, person.tg_user, person.city, person.type_pet, person.name_pet, person.sex_pet, person.breed_pet, person.describe, person.photo)  
    session.add(human)  
    session.commit()  
